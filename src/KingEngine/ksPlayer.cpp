	...
	: m_rect(rect), m_vel_x(0.f), m_vel_y(0.f), m_type(0), m_frame(0)
{
	...
	m_quad.setPrimitiveType(sf::Triangles);
	m_quad.resize(6);
	...
	m_quad[4].position = sf::Vector2f(rect.x, rect.y);
	m_quad[5].position = sf::Vector2f(rect.x + rect.w, rect.y + rect.h);
	...
	m_quad[4].texCoords = sf::Vector2f(0, 0);
	m_quad[5].texCoords = sf::Vector2f(rect.w / 2, rect.h / 2);
}
...
void ksPlayer::animate(int type_low, int type_high, int speed)
{
	if (m_type < type_low || m_type > type_high)
	{
		...
		m_quad[4].texCoords = sf::Vector2f(tsrect.x, tsrect.y);
		m_quad[5].texCoords = sf::Vector2f(tsrect.x + tsrect.w, tsrect.y + tsrect.h);
	}
